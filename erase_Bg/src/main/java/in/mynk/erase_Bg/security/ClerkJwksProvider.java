...
public class ClerkJwksProvider {
    ...
    private static final long CACHE_TTL = 3600000;//1 hr
    ...
    private void refreshKeys() throws Exception {
        ...
        for (JsonNode keyNode : keys) {
            if (keyNode.has("kid") && keyNode.has("kty") && keyNode.has("alg") &&
                    keyNode.has("n") && keyNode.has("e")) {
    ...
    private PublicKey createPublicKey(String modulus, String exponenet) throws Exception {
        byte[] modulusBytes = Base64.getUrlDecoder().decode(modulus);
        byte[] exponentBytes = Base64.getUrlDecoder().decode(exponenet);
